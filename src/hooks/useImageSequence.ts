...
function getIsMobile(): boolean {
  ...
  return window.innerWidth < HERO_MOBILE.breakpoint;
}
...
export function useImageSequence(): UseImageSequenceReturn {
  ...
  useEffect(() => {
    ...
    const batchSize = mobile ? HERO_MOBILE.batchSize : 6;
    ...
    async function loadFrame(index: number): Promise<void> {
      try {
        const response = await fetch(getFramePath(index, mobile));
    ...
    async function preload() {
      ...
      const remaining = Array.from(
        ...
        (i) =>
          !(HERO_CONFIG.criticalFrames as readonly number[]).includes(i),
      );
